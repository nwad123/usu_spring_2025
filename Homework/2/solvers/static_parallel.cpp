...
[[nodiscard]] auto StaticParallel::operator()(const std::span<fp> dataset) const -> Bin
{
    using std::thread;
    using std::ref;

    auto task = [&dataset](/*in*/ size_t id, /*in*/ const std::span<fp> ranges, /*out*/ Bin &bin) {
    ...
    std::array<fp, config.bins> ranges = {};
    fp f{ config.max };
    ...
    std::array<Bin, config.threads> bins;
    std::array<thread, config.threads> threads;

    for (size_t id = 0; id < config.threads; id++) {
        threads[id] = thread{ task, id, std::span{ ranges }, ref(bins[id]) };
    }
